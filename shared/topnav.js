// ReliToolbox v0.1 — Persistent top navigation (ChemCalc-style).
// Injected into every page that includes this script. Auto-highlights current section by URL.
// Zero dependencies. Self-contained styling.
(function(global){
  const GROUPS = [
    { id:'data',   zh:'数据分析',     en:'Data Analysis',   paths:['/weibull/','/amsaa/'] },
    { id:'system', zh:'系统建模',     en:'System Modeling', paths:['/rbd/','/rbd_mcs/','/fta/','/ram/','/alloc/'] },
    { id:'risk',   zh:'风险 · 安全',  en:'Risk & Safety',    paths:['/fmea/','/sil/','/rbi/'] },
    { id:'maint',  zh:'维修 · 成本',  en:'Maint & Cost',    paths:['/pm/','/spa/','/lcc/','/equipment/','/predict/'] },
  ];
  function currentGroup(){
    const p = location.pathname;
    for(const g of GROUPS){ if(g.paths.some(x=>p.includes(x))) return g.id; }
    return null;
  }
  function toBase(n){
    // Depth from root — all module pages are at depth 1 (one folder deep) except root
    const depth = location.pathname.replace(/\/[^/]*$/,'').split('/').filter(Boolean).length;
    return depth>0 ? '../' : './';
  }
  function render(){
    const lang = (localStorage.getItem('rtLang')||'zh');
    const base = toBase();
    const cur  = currentGroup();
    const firstPath = g => (g.paths[0].replace(/^\//,''));
    const tabs = GROUPS.map(g=>
      `<a class="rt-tab${cur===g.id?' on':''}" href="${base}${firstPath(g)}index.html">${lang==='zh'?g.zh:g.en}</a>`
    ).join('');
    const nav = document.createElement('div');
    nav.className = 'rt-topnav';
    nav.innerHTML =
      `<div class="rt-topnav-in">
  ...
  function injectStyle(){
    if(document.getElementById('rt-topnav-css')) return;
    const s = document.createElement('style'); s.id='rt-topnav-css';
    s.textContent = `
